...
- `script-execute`: код в **`csharpCode`** (НЕ `code`). Full-code режим: класс +
  **static**-метод (instance → "Non-static method requires a target"). **Имена
  класса/метода — ПРОИЗВОЛЬНЫЕ**, но их надо передать в `className`+`methodName`
  (не обязательно `Script`/`Main`). Проверено: `_LbRun.Run`, `_Dedupe.Run` работают.
  Шаблон:
  ```json
  {"csharpCode":"using UnityEngine; public static class _Foo { public static void Run(){ /* ... */ } }",
   "className":"_Foo","methodName":"Run"}
  ```
  Возврат строки удобен — приходит в `result.value` (void → `"Success"`).
  Многострочный/со спецсимволами код → класть JSON в файл и `--input-file <file>`
  (а НЕ stdin-пайп: длинный escaped-код в pwsh-строке ломается).
  Для Editor-API (`AssetDatabase`, `SerializedObject`) — `using UnityEditor;` ок,
  скрипт исполняется в Editor-контексте.
...
- `gameobject-find`: цель — в **`gameObjectRef`** (объект `{name|path|instanceID}`),
  НЕ голым `name`/`path` на верхнем уровне. `{"name":"Foo"}` верхним уровнем →
  пустой `text:null` ответ (молча, без ошибки). Правильно:
  `{"gameObjectRef":{"name":"Foo"},"includeComponents":true,"includeHierarchy":true,"hierarchyDepth":2}`.
  Поиск по `name` ищет в **активной** сцене; если объект — ребёнок, надёжнее `path`
  (`"Canvas/PausePanel"`). Дочерние компоненты: `includeHierarchy` + `hierarchyDepth`.

- **Переименовать GameObject / сменить значение поля-свойства** (`name`, TMP `m_text`):
  через **`jsonPatch`** в `object-modify`/`gameobject-component-modify`, НЕ через
  `objectDiff`/`gameObjectDiffs` с `name` на верхнем уровне. Верхний `name` в diff =
  **идентификатор** объекта, не сеттер → "No fields modified" / дамп всех данных.
  Правильно: `{"objectRef":{"instanceID":N},"jsonPatch":{"name":"StartPanel"}}`.
  TMP-текст: `m_text` (сериализованное поле) через `jsonPatch`, НЕ `text` и НЕ
  `props`-запись `m_text` (даёт "Property 'm_text' not found or not writable").
  Поля-ссылки компонента (`_clip`) — наоборот, через `componentDiff.fields[]` +
  `{"instanceID":N}` (работает, проверено).

- **Изменение сцены/иерархии падает в Play Mode**: `gameobject-duplicate`/`create`/
  `modify` → "This cannot be used during play mode." ПЕРЕД правкой сцены проверить
  `editor-application-get-state` → если `IsPlaying:true`, остановить
  `{"isPlaying":false}` и дождаться `IsPlaying:false`. Оператор мог оставить Play.

- `gameobject-duplicate` возвращает в ответе **ссылку оригинала**, не копии.
  Копия называется `"<Имя> (1)"` — искать её отдельным `gameobject-find` по этому
  имени, не полагаться на возвращённый instanceID.

- **InstanceID переживает выход из Play Mode** (проверено: до/после стоп-плея ID
  панелей те же). Но НЕ переживает рестарт Editor / domain reload. В пределах сессии
  без перезагрузок — стабильны; для долгих операций надёжнее `path`/`name`.
...
- `screenshot-game-view` отдаёт PNG как **base64 в JSON** (через CLI) — извлекать в
  файл и читать как картинку. И главное: **в Edit Mode скрин закеширован/мёртвый**
  (или "render texture not available" если вкладка Game закрыта) — правки на сцене не
  видны. Свежий кадр — только в **Play Mode** или просить скрин у оператора. Подробно
  про проверку верстки → `docs/ui-conventions.md`.

- `editor-application-set-state`: вход через **`state`** (`"play"`/`"edit"`/`"pause"`)
  ИЛИ `isPlaying`-bool — оба видел рабочими. Запуск Play **без кликов** работает НЕ
  всегда: иногда `IsPlaying` остаётся false (нужен фокус окна Unity) — тогда просить
  оператора нажать Play руками.
...
## Вызов из PowerShell — формат и permission-allowlist

На Windows вызовы идут через **PowerShell**, JSON-аргумент — болезненная точка:

- **Не инлайнить JSON в `--input`** прямо в pwsh-строке: кавычки `"` ломаются
  (`--input must be valid JSON`, `Invalid JSON at position 1`). Экранирование
  `\"`/`\\\"` нечитаемо и хрупко.
- **Канон: stdin-пайп здесь-строкой или одинарными кавычками** →
  `'{"json":true}' | npx --yes unity-mcp-cli run-tool <tool> --input-file -`.
  `--input-file -` читает stdin. Одинарные кавычки в pwsh = литерал, `$`/`"` целы.

- **ГРАБЛЯ allowlist (жрёт время на промптах):** permission-правила матчат по
  **префиксу строки команды**. Stdin-пайп даёт УНИКАЛЬНЫЙ префикс на каждый вызов
  (`'{...свой json...}' | npx ...`) → prefix-правило `Bash(npx unity-mcp-cli *)` НЕ
  ловит, каждый вызов = новый промпт. Решения:
  1. Широкие wildcard-правила в `.claude/settings.local.json` с `*` В НАЧАЛЕ:
     `"PowerShell(*npx --yes unity-mcp-cli *)"`, `"PowerShell(*unity-mcp-cli run-tool *)"`.
  2. **Новые правила harness читает на СТАРТЕ сессии.** Дописал в середине —
     watcher не подхватит, пока не открыть `/permissions` (перечитывает конфиг) или
     рестарт. `/permissions` может быть НЕдоступен в headless/IDE-окружении →
     тогда либо рестарт, либо жать Allow вручную на оставшиеся вызовы.
  3. Вывод — это означает: **класть широкие MCP-allow в settings.local.json ЗАРАНЕЕ**,
     один раз, до начала работы. Не дописывать по ходу.
...
- **`console-get-logs` отдаёт СТЕЙЛ compiler-warnings.** Unity console хранит
  warning'и/ошибки последних компиляций и чистит их только при НОВОЙ компиляции,
  НЕ при `console-clear-logs` (та чистит лишь MCP-кэш collector'а). После фикса
  warning в коде лог может ещё показывать старый — с устаревшими номерами строк
  (файл уже короче/длиннее) или со старыми сигнатурами твоих промежуточных правок.
  Как отличить реальный warning от стейла: сверить номер строки в логе с
  **актуальным** содержимым файла (Read). Если `assets-refresh` вернул success и
  `editor-application-get-state` показывает `IsCompiling:false` без новой компиляции —
  скорее всего перекомпиляции не было, лог стейл, исходник уже верный. Не гонять
  refresh по кругу; warning уйдёт при первой реальной recompile (фокус Editor/Play/
  правка любого .cs).
...
- `FindObjectsByType<T>(FindObjectsSortMode)` (1-арг с SortMode) — **deprecated**
  (CS0618). И 2-арг `(FindObjectsInactive, FindObjectsSortMode)` тоже. Без warning:
  `FindObjectsByType<T>()` или `FindObjectsByType<T>(FindObjectsInactive.Exclude)`.
...
`script-execute` — всё прошло БЕЗ единого клика в Unity, окно вне фокуса.

**Исключения, где окно Unity руками ВСЁ ЖЕ нужно:**
- Переключение Connection Cloud→Custom (один раз после установки/реимпорта плагина).
- **Вход в Play Mode через `editor-application-set-state`** — наблюдалось, что
  `IsPlaying` остаётся false без фокуса окна (set-state «успех», но playmode не
  стартует). Тогда просить оператора нажать Play (Ctrl+P).
- **Game View скрин в Edit Mode** — мёртвый/кешированный без фокуса (см. выше).
