...
- **Шкала гнева — float 0..`_angerMax`(100)**, красный radial-ринг вокруг иконки кары над алтарём. Изменения: +`_angerPerSec`(4)/сек пассивно (копится только пока алтарь активен, не во время Waiting/rage), сдал задание −`_angerOnDeliver`(10).
...
- **Altar** (задание): `_resourceSteps`(1 — все предметные квесты = 1 предмет), `_runZoneCount`(4), `_startDelay`(стаггер старта: Altar_0=0, _1=10, _2=20, _3=30, _4=40с). Рефилл мгновенный — новое задание сразу, пузырь не прячется.
